...
module.exports = {
	...
	onStart: async function ({ api, message, event, args, getLang }) 
  {
    ...
    if(keySearch.includes("-") == false) return api.sendMessage('❌️Please enter in the format, example: Pinterest Naruto -5 (it depends on you how many images you want to appear in the result)❌️', event.threadID, event.messageID)
    ...
    api.sendMessage({
        attachment: imgData,
        body: numberSearch + ' PICS OF : '+ keySearchs
    }, event.threadID)
